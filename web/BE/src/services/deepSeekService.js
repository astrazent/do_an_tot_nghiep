...
async function fetchBatchSummary(batch, startIndex = 0) {
    const allText = batch
        ...
        .map((c, idx) => {
            const content = c.content.trim().slice(0, 300)
    ...
    try {
        ...
        for (let i = 0; i < batches.length; i++) {
            ...
            ;['product', 'service'].forEach(key => {
                ...
                finalSummary[key].positiveCount =
                    finalSummary[key].positive.length
                finalSummary[key].negativeCount =
                    finalSummary[key].negative.length
            })
        }
