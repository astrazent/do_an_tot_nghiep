...
const getByIdService = async postCategoryId => {
    const postCategoryLink =
        await PostCategoriesModel.getLinkById(postCategoryId)
    if (!postCategoryLink) {
        throw new ApiError(
...
const getListService = async filters => {
    const postCategoryLinks = await PostCategoriesModel.listLinks(
        filters.limit,
        filters.offset
    )
...
const getListCategoryByPostService = async postId => {
    ...
    for (let category of categories) {
        const categoryTitle = await CategoriesModel.getCategoryById(
...
const updateService = async (postCategoryId, data) => {
    ...
    const updatedLink = await PostCategoriesModel.updateLink(
        postCategoryId,
        data
    )
    return updatedLink
