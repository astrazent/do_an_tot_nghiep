                ...
                <div className="flex items-start justify-between">
                    <div>
                        <div className="flex items-center gap-2 mt-1">
                            ...
                            <span
                                className={`px-2 py-0.5 text-xs rounded font-medium ${product.status ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
                            >
                                {product.status
                                    ? 'Đang kinh doanh'
                                    : 'Ngừng kinh doanh'}
                ...
                {product.images && product.images.length > 0 && (
                    <div className="flex gap-2 overflow-x-auto pb-2">
                        {product.images.map((img, i) => (
                            <div
                                key={i}
                                className="w-20 h-20 bg-gray-100 rounded-lg flex-shrink-0 flex items-center justify-center text-xs text-gray-400"
                            >
                                Img {i + 1}
                            </div>
                        ))}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-gray-50 p-4 rounded-xl border border-gray-100">
                    ...
                    <div>
                        ...
                        <div className="text-lg font-medium text-gray-600 line-through">
                            {formatCurrency(product.origin_price)}
                        </div>
                    </div>
                    <div>
                        <div className="text-xs text-gray-500 mb-1">
