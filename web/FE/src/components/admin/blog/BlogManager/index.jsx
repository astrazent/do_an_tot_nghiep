// src/admin/blog/BlogManager.jsx
...
import { getListPost } from '~/services/admin/postAdminService'
          ...
          {statuses.map((item) => (
            <div
              ...
              className={`px-4 py-2.5 text-sm cursor-pointer transition-colors ${
                value === item.key ? 'bg-indigo-50 text-indigo-600 font-medium' : 'text-gray-600 hover:bg-gray-50'
              }`}
            >
              {item.label}
            </div>
...
const BlogManager = () => {
  ...
  const handleToggleStatus = (postId) => {
    setPosts((prevPosts) =>
      prevPosts.map((post) =>
        post.id === postId ? { ...post, status: post.status === 'Đăng' ? 'Ẩn' : 'Đăng' } : post
      )
    )
  ...
  const currentPosts = filteredPosts.slice((currentPage - 1) * displayPerPage, currentPage * displayPerPage)
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Detail Modal */}
      <PostDetailModal post={selectedPost} isOpen={isDetailModalOpen} onClose={handleCloseDetailModal} />
