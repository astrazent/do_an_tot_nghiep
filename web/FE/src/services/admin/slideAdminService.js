...
export const getListSlider = async () => {
    try {
        const response = await axios.get('http://localhost:2000/v1/slider/list')
...
export const createSlider = async (data) => {
    try {
        const response = await axios.post('http://localhost:2000/v1/slider', data)
        return response.data
    } catch (error) {
        console.error('Đã xảy ra lỗi khi lấy dữ liệu', error)
        throw error
...
export const updateSlider = async (data, sliderId) => {
    try {
        const response = await axios.patch(
            `http://localhost:2000/v1/slider/${sliderId}`, 
            data,
            {
                headers: {
