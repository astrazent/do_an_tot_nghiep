...
const getListOrderItemsService = async data => {
    const listOrderItems = await OrderItemsModel.listOrderItems(
        data.limit,
        data.offset
    )
...
const getItemsByTransactionService = async transaction_id => {
    const listItemsByTransaction =
        await OrderItemsModel.getItemsByTransaction(transaction_id)
...
const getItemsByProductService = async product_id => {
    const listItemsByProduct =
        await OrderItemsModel.getItemsByProduct(product_id)
