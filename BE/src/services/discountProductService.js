...
const getDiscountProductByIdService = async discountProductId => {
    const discountProduct = await DiscountProductsModel.getLinkById(discountProductId)
    if(!discountProduct) {
        throw new ApiError(
...
const getListDiscountProductService = async data => {
    const discountProduct = await DiscountProductsModel.listLinks(data.limit, data.offset)
    return discountProduct
}
...
const updateDiscountProductService = async (discountProductId, data) => {
    ...
    const result = await DiscountProductsModel.updateLink(discountProductId,data)
    return result
}
...
const getProductsByDiscountService = async discount_id => {
    const listProductByDiscount = await DiscountProductsModel.getProductsByDiscount(discount_id)
...
const getDiscountsByProductService = async product_id => {
    const listDiscountByProduct = await DiscountProductsModel.getDiscountsByProduct(product_id)
